(function() {
	...
	function fillInCosts(e) {
		let box = $($(e.target).parents().filter(".row"));
		let trait = box.data().trait;
		let cost;
	...
	function getAttrCost(box) {
		...
		let levelBox = box.find(".level");
		let level = parseInt(levelBox.val()) || 0;
		let position = box.data().position;
	...
	function getSkillCost(box) {
		...
		let skillType = box.find(".type").val();
		let difficulty = box.find(".difficulty").val();

		if (!level || !skillType || !difficulty) {
			return 0;
		}

		let skillLevel = $(`#attr-type-${skillType}`).find(".level").val() || 0;
	...
	function fillInAttr2Values(levels) {
		...
		for (let i = 0; i < attr2Rows.length; i++) {
			let row = $(attr2Rows[i]);
			let level = levels[row.data().abbr];
